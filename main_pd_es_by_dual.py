...
parser.add_argument('--dual_momentum', type = float, default=0.0, help = 'Dual Update Momentum')
...
# Update duals
def dual_update(train_loader, model, lambdas, grad_lambdas, u, h):
    model.eval()
    with torch.no_grad():
        for i, (images, labels, indexes) in enumerate(train_loader):
            ...
            # Dual Update
            grad_lambdas[indexes] = args.dual_momentum*grad_lambdas[indexes] + slacks
            lambdas[indexes] = F.relu(lambdas[indexes] + args.dual_lr*grad_lambdas[indexes])
    return lambdas, u, grad_lambdas
# Evaluate the Model
...
# train models
    ...
    # Update duals
    print(f"dual threshold: {args.dual_thresh}")
    print(f"mean u: {torch.mean(u)}")
    print(f"lambdas: {torch.mean(lambdas)}")
    lambdas, u, grad_lambdas = dual_update(train_loader, model, lambdas, grad_lambdas, u, h)
    print(f"mean u: {torch.mean(u)}")
    print(f"lambdas: {torch.mean(lambdas)}")
    # evaluate models
